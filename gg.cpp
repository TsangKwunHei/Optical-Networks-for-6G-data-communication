...
std::vector<std::vector<std::pair<int, int>>> adjacency_list; // Adjacency list of the network graph (neighbor node, edge ID)
std::vector<std::pair<int, int>> edges;                    // List of edges (edge index to node pairs)
std::vector<std::bitset<MAX_K + 1>> edge_wavelengths;      // Edge ID to occupied wavelengths
std::vector<std::bitset<MAX_K + 1>> initial_edge_wavelengths; // Initial wavelengths on edges
int K = 0; // Number of initial services
std::vector<Service> services; 
int T1 = 0;  
std::vector<FailureScenario> failure_scenarios; // Edge failure scenarios we generate
...
std::vector<int> AssignWavelengthsUsingGA(const std::vector<int>& affected_services);
bool RunGAWithTemp(Service& srv, std::vector<std::bitset<MAX_K + 1>>& temp_edge_wavelengths, std::vector<int>& temp_Pi);
double FitnessFunction(const std::vector<int>& chromosome, const Service& srv, const std::vector<std::bitset<MAX_K + 1>>& temp_edge_wavelengths, const std::vector<int>& temp_Pi);
std::vector<int> GenerateInitialChromosome(const Service& srv);
std::vector<int> MutateChromosome(const std::vector<int>& chromosome, const Service& srv);
std::pair<std::vector<int>, std::vector<int>> CrossoverChromosomes(const std::vector<int>& parent1, const std::vector<int>& parent2, const Service& srv);
bool DecodeChromosomeWithTemp(const std::vector<int>& chromosome, const Service& srv, std::vector<int>& path, std::vector<int>& wavelengths, std::map<int, int>& converters_needed, const std::vector<std::bitset<MAX_K + 1>>& temp_edge_wavelengths, const std::vector<int>& temp_Pi);
std::vector<std::pair<int, int>> GetAvailableIntervalsWithTemp(int edge_id, int width, const Service& srv, const std::vector<std::bitset<MAX_K + 1>>& temp_edge_wavelengths);
...
public:
    ...
    explicit MaxFlow(int N_) : N(N_), graph(N_ + 1), level(N_ + 1, -1), ptr(N_ + 1, 0) {}
...
double JaccardSimilarity(const std::set<int>& a, const std::set<int>& b) {
    ...
    int union_size = static_cast<int>(a.size()) + static_cast<int>(b.size()) - intersection_size;
...
void GenerateFailureScenarios() {
    ...
    std::sort(service_indices.begin(), service_indices.end(), [&](int a, int b) {
...
std::vector<int> DetectAffectedServices(int e) {
    ...
    for (int i = 1; i <= K; ++i) {
        ...
        if (std::find(services[i].path.begin(), services[i].path.end(), e) != services[i].path.end()) {
...
std::vector<int> AssignWavelengthsUsingGA(const std::vector<int>& affected_services) {
    ...
    std::sort(sorted_services.begin(), sorted_services.end(), [&](int a, int b) {
        return services[a].V > services[b].V;
    });
...
bool RunGAWithTemp(Service& srv, std::vector<std::bitset<MAX_K + 1>>& temp_edge_wavelengths, std::vector<int>& temp_Pi) {
    ...
    for (int i = 0; i < POPULATION_SIZE; ++i) {
        ...
        fitness_values.emplace_back(FitnessFunction(chromosome, srv, temp_edge_wavelengths, temp_Pi));
    }
    ...
    for (int generation = 0; generation < MAX_GENERATIONS; ++generation) {
        ...
        std::uniform_int_distribution<> dist_idx(0, static_cast<int>(population.size()) - 1);
        ...
        while (static_cast<int>(new_population.size()) < POPULATION_SIZE) {
            ...
            const std::vector<int>& parent1 = (fitness_values[idx1] < fitness_values[idx2]) ? population[idx1] : population[idx2];

            // Tournament selection for parent2
            idx1 = dist_idx(gen);
            idx2 = dist_idx(gen);
            const std::vector<int>& parent2 = (fitness_values[idx1] < fitness_values[idx2]) ? population[idx1] : population[idx2];
            ...
            if (!child1.empty()) new_population.emplace_back(child1);
            if (!child2.empty()) new_population.emplace_back(child2);
        }
        ...
        for (const auto& chrom : population) {
            fitness_values.emplace_back(FitnessFunction(chrom, srv, temp_edge_wavelengths, temp_Pi));
        }
    }

    // Get the best chromosome
    auto best_iter = std::min_element(fitness_values.begin(), fitness_values.end());
    int best_idx = static_cast<int>(std::distance(fitness_values.begin(), best_iter));
...
double FitnessFunction(const std::vector<int>& chromosome, const Service& srv, const std::vector<std::bitset<MAX_K + 1>>& temp_edge_wavelengths, const std::vector<int>& temp_Pi) {
    ...
    // Fitness considers path length and number of wavelength changes (channel conversions)
...
// Function to generate an initial chromosome for a service using BFS for a single path
std::vector<int> GenerateInitialChromosome(const Service& srv) {
    ...
    while (!q.empty()) {
        std::vector<int> path_nodes = q.front(); q.pop();
...
std::vector<int> MutateChromosome(const std::vector<int>& chromosome, const Service& srv) {
    ...
    std::uniform_int_distribution<> dist_idx(0, static_cast<int>(mutated.size()) - 1);
    ...
    if (possible_edges.empty()) return mutated;

    // Replace with a random adjacent edge
    std::uniform_int_distribution<> dist_edge(0, static_cast<int>(possible_edges.size()) - 1);
...
std::pair<std::vector<int>, std::vector<int>> CrossoverChromosomes(const std::vector<int>& parent1, const std::vector<int>& parent2, const Service& srv) {
    ...
    if (parent1.empty() || parent2.empty()) return { child1, child2 };

    std::uniform_real_distribution<> dis(0.0, 1.0);
    if (dis(gen) < CROSSOVER_RATE) {
        std::uniform_int_distribution<> dist_point(0, static_cast<int>(std::min(parent1.size(), parent2.size())) - 1);
...
// Function to get available intervals of wavelengths of size 'width' on an edge using temporary edge_wavelengths
std::vector<std::pair<int, int>> GetAvailableIntervalsWithTemp(int edge_id, int width, const Service& srv, const std::vector<std::bitset<MAX_K + 1>>& temp_edge_wavelengths) {
    ...
    while (true) {
        ...
        for (int t = 0; t < T; ++t) {
            ...
            while (true) {
                ...
                std::vector<int> successfully_replanned_services = AssignWavelengthsUsingGA(affected_services);
                ...
                for (const auto& service_id : successfully_replanned_services) {
                    ...
                    for (std::size_t j = 0; j < srv.path.size(); ++j) {
                        std::cout << srv.path[j] << " " << srv.wavelengths[j] << " " << srv.wavelengths[j] + (srv.R - srv.L) << (j < srv.path.size() - 1 ? " " : "\n");
                    }
                }
                std::cout.flush();
            }
