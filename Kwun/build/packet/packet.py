...
def main():
    ...
    for slice_id in range(n):
        ...
        for pkt_id in range(m_i):
            ...
            packet = Packet(ts, pkt_size, pkt_id, slice_id, deadline)
        ...
        slice_info = Slice(
            ...
            SliceBW_i,
            slice_id,
            deque(sorted(packets, key=lambda x: x.ts)),
            0,
            packets[0].ts if packets else 0,
            0,
            0,
        )
    ...
    current_time = 0
    port_available_time: int = 0
    packet_idx = 0
    heap: list[tuple[tuple[int, float, int], Packet]] = []

    # Priority function: (earliest deadline, highest SliceBW, earliest slice_id)
    def get_priority(pkt: Packet):
        return (
            pkt.deadline,
            -slices[pkt.slice_id].SliceBW_i,
            slices[pkt.slice_id].slice_id,
        )
    ...
    while packet_idx < total_packets or heap:
        ...
        if not heap:
            ...
            if packet_idx < total_packets:
                current_time = max(
                    current_time, all_packets[packet_idx].ts, port_available_time
                )
            else:
                break
            continue
